...
export function BeritaDetail() {
  ...
  const item = itemIndex !== -1 ? news[itemIndex] : null;

  const prevItem = itemIndex > 0 ? news[itemIndex - 1] : null;
  const nextItem = itemIndex !== -1 && itemIndex < news.length - 1 ? news[itemIndex + 1] : null;

  const relatedNews = news.filter((n) => n.id !== id).slice(0, 5);

  if (!item) {
    return (
      <section className="mx-auto max-w-3xl px-4 py-24 text-center sm:px-6 lg:px-8">
        <h1 className="font-display text-2xl font-bold text-brand-blue-deep dark:text-white">Berita tidak ditemukan</h1>
        ...
        <Link to="/research?tab=berita" className="mt-6 inline-flex items-center gap-2 rounded-full bg-brand-blue px-6 py-2.5 text-sm font-semibold text-white">
    ...
    <div className="bg-brand-cream py-10 dark:bg-[#0b1329]">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6 flex items-center justify-between">
          <Link
            to="/research?tab=berita"
            className="inline-flex items-center gap-2 text-sm font-semibold text-brand-blue dark:text-brand-teal hover:underline"
          >
            <ArrowLeft className="h-4 w-4" /> Kembali ke Berita & Kegiatan
          </Link>

          {/* Trending Banner Tag */}
          <div className="hidden items-center gap-2 rounded-lg bg-white px-3 py-1 text-xs shadow-sm dark:bg-slate-900 sm:flex">
            <span className="flex items-center gap-1 rounded bg-brand-red px-2 py-0.5 font-bold uppercase text-white">
              <TrendingUp className="h-3 w-3" /> Trending
            </span>
            <span className="truncate max-w-[280px] text-muted-foreground dark:text-slate-300">{item.title}</span>
          </div>
        </div>
        ...
        <div className="grid gap-10 lg:grid-cols-12">
          ...
          <article className="rounded-3xl border border-border bg-white p-6 shadow-sm dark:bg-[#152238] dark:border-slate-800 sm:p-10 lg:col-span-8">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2 border-y border-border/60 py-3.5 text-xs text-muted-foreground dark:text-slate-300 sm:text-sm">
              <span className="flex items-center gap-1.5 font-medium text-brand-blue-deep dark:text-slate-100">
                <User className="h-4 w-4 text-brand-blue" /> {item.source || "INCLUSA Institute"}
              </span>
              <span className="flex items-center gap-1.5">
                <Tag className="h-4 w-4 text-brand-coral" /> {item.tag}
              </span>
              <span className="flex items-center gap-1.5">
                <Calendar className="h-4 w-4 text-brand-teal" /> {item.date}
              </span>
            </div>
            ...
            <div className="mt-8 space-y-6 text-base leading-relaxed text-slate-800 dark:text-slate-200 sm:text-lg">
              {paragraphs.map((p, idx) => (
                <p key={idx} className="leading-relaxed">
                  {idx === 0 && (
                    <strong className="font-bold text-brand-blue-deep dark:text-white uppercase mr-1">
            ...
            <div className="mt-10 border-t border-border/80 pt-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <span className="flex items-center gap-2 font-display text-sm font-bold text-brand-blue-deep dark:text-white">
                  <Share2 className="h-4 w-4 text-brand-blue" /> Bagikan Berita Ini:
                </span>
                <div className="flex flex-wrap gap-2">
                  <a
                    href={`https://wa.me/?text=${encodeURIComponent(`${item.title} - ${currentUrl}`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
            ...
            <div className="mt-10 grid gap-4 border-t border-border/80 pt-6 sm:grid-cols-2">
              {prevItem ? (
                ...
                >
                  <span className="flex items-center gap-1 text-xs font-semibold text-muted-foreground group-hover:text-brand-blue">
                    <ChevronLeft className="h-3.5 w-3.5" /> Berita Sebelumnya
                  </span>
              ...
              {nextItem ? (
                ...
                >
                  <span className="flex items-center gap-1 text-xs font-semibold text-muted-foreground group-hover:text-brand-blue">
                    Berita Selanjutnya <ChevronRight className="h-3.5 w-3.5" />
                  </span>
                  <span className="mt-1 line-clamp-1 font-semibold text-brand-blue-deep dark:text-white text-sm">
                    {nextItem.title}
                  </span>
          ...
          {/* RIGHT 4 COLUMNS: SIDEBAR (RELATED NEWS & TOPICS) */}
          <aside className="space-y-6 lg:col-span-4">
            
            {/* Berita Terkini / Latest News List */}
            <div className="rounded-3xl border border-border bg-white p-6 shadow-sm dark:bg-[#152238] dark:border-slate-800">
              <h3 className="flex items-center gap-2 font-display text-lg font-bold text-brand-blue-deep dark:text-white">
                <Newspaper className="h-5 w-5 text-brand-blue" /> Berita Terkini
              </h3>
                  ...
                  >
                    {n.image && (
                      <div className="h-16 w-20 shrink-0 overflow-hidden rounded-lg">
                        <ImageWithFallback src={n.image} alt={n.title} className="h-full w-full object-cover transition-transform group-hover:scale-105" />
                      </div>
                    )}
                    <div className="min-w-0 flex-1">
                      <span className="text-[0.65rem] font-bold uppercase tracking-wider text-brand-coral">{n.tag}</span>
                      <h4 className="mt-0.5 line-clamp-2 text-xs font-bold text-brand-blue-deep dark:text-white group-hover:text-brand-blue">
                        {n.title}
                      </h4>
                      <p className="mt-1 text-[0.68rem] text-muted-foreground dark:text-slate-400">{n.date}</p>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            <div className="rounded-3xl border border-border bg-white p-6 shadow-sm dark:bg-[#152238] dark:border-slate-800">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {["Pemberdayaan Anak", "Perempuan", "Disabilitas", "Kebijakan Publik", "Riset Inklusi", "Sidoarjo", "Kesehatan"].map((t) => (
                  <span
                    key={t}
            ...
            <div className="rounded-3xl bg-gradient-to-br from-brand-blue via-brand-blue-dark to-brand-blue-deep p-6 text-white shadow-md">
              ...
              <h4 className="mt-3 font-display text-lg font-bold text-white">
                Kemitraan & Riset INCLUSA
              </h4>
              <p className="mt-2 text-xs leading-relaxed text-white/85">
                INCLUSA Institute membuka peluang kerja sama riset, publikasi, dan advokasi kebijakan publik.
              </p>
              <Link
                to="/kontak"
